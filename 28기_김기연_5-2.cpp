...
void VendingMachine::purchase(double v1) // 총 금액 출력, 지폐 입력, 거스름돈 출력
{
	...
	while (1)
	{
		...
		int change = money - v1; // change는 입력된 지폐 - 총 금액 (거스름돈)

		if (money < v1)
			cout << "금액이 부족합니다. 다시 넣어주세요. : "; // 다시 money 입력받기

		else if (money > v1)
		{
			if (change % 100 == 0)
				cout << endl << "거스름돈 입니다." << endl;

			else
				cout << endl << "거스름돈은 500원과 100원만 가능합니다." << endl;
		...
		{
			cout << endl << "거스름돈이 없습니다. 안녕히가세요." << endl;
			break;
		}
	}
}

void VendingMachine::menu() // 새로운 메뉴판
{
	cout << "===== SWING 자판기 =====" << endl;
